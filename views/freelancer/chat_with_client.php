...
<script>
// Function to load messages between freelancer and client
function loadMessages() {
    const clientId = new URLSearchParams(window.location.search).get('client_id');
    const freelancerId = <?= $_SESSION['freelancer_id'] ?>; // Use the freelancer ID from session

    fetch(`index.php?controller=freelancer&action=loadMessages&client_id=${clientId}&freelancer_id=${freelancerId}`)
        ...
        .then(data => {
            ...
            data.forEach(msg => {
                ...
                messageDiv.classList.add(msg.sender_type === 'freelancer' ? 'sent' : 'received');
                messageDiv.innerHTML = `
                    <p><strong>${msg.sender_type === 'freelancer' ? 'You' : 'Client'}:</strong></p>
                    <p>${msg.message}</p>
...
document.getElementById('message-form').addEventListener('submit', function(event) {
    ...
    const clientId = new URLSearchParams(window.location.search).get('client_id');
    const freelancerId = <?= $_SESSION['freelancer_id'] ?>;

    fetch('index.php?controller=freelancer&action=sendMessage', {
        method: 'POST',
